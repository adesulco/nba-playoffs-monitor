...
import { CLUBS_BY_ESPN_ID } from '../lib/sports/liga-1-id/clubs.js';
import { readCache, writeCache } from '../lib/swrCache.js';

function cacheKey(daysBack, daysFwd) {
  return `superleague-fixtures-${daysBack}-${daysFwd}`;
}
const CACHE_TTL = 2 * 60 * 1000;

/**
 * Indonesian Super League 2025-26 fixtures + recent results via ESPN scoreboard.
 *
 * Endpoint:
 *   GET /api/proxy/espn/soccer/idn.1/scoreboard?dates=YYYYMMDD-YYYYMMDD
 *
 * Verified live 2026-04-26 — returns events array with home/away competitors,
 * scores, and status (pre/in/post). Same shape as eng.1.
 *
 * Default window: 7 days back, 7 days forward — matches EPL hook.
 */
...
export function useSuperLeagueFixtures({ daysBack = 7, daysFwd = 7 } = {}) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const url = `/api/proxy/espn/soccer/idn.1/scoreboard?dates=${ymd(from)}-${ymd(to)}`;
