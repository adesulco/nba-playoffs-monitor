...
export default function F1Race() {
  ...
  // Catatan Race — if the race is already past, three recap PNGs exist
  // (generated by scripts/generate-f1-recap.mjs and committed to public/):
  //   /og/f1/2026-R{nn}-{slug}-og.png      1200×630  (link preview)
  //   /og/f1/2026-R{nn}-{slug}-story.png   1080×1920 (IG / WA Story)
  //   /og/f1/2026-R{nn}-{slug}-square.png  1080×1080 (IG feed)
  //
  // The og variant feeds <SEO image>; the story variant feeds the
  // ShareButton's "Save to IG Story" downloader (v0.13.9 wires this
  // — previously the story PNG was generated but never surfaced).
  const todayISO = new Date().toISOString().slice(0, 10);
  const isPast = gp.dateISO <= todayISO;
  const recapBase = `https://www.gibol.co/og/f1/2026-R${String(gp.round).padStart(2, '0')}-${gp.slug}`;
  const recapOgUrl = isPast ? `${recapBase}-og.png` : null;
  const recapStoryUrl = isPast ? `${recapBase}-story.png` : null;
  ...
  // subrow. Picker label = R-badge (F1-red branded tile) + GP
  // name per directive §4 ("R-badge + GP name move into
  // StatusStrip as picker label `★ R06 · Monaco GP ▾`").
  useEffect(() => {
    setTopbarSubrow(
      <HubStatusStrip
        srOnlyTitle={`${gp.name} 2026 · Round ${gp.round} · ${gp.circuit}, ${gp.country}`}
        accent={F1_RED}
        picker={(
          <Link
            to="/formula-1-2026"
            style={{
              ...
              background: `${F1_RED}22`,
              color: 'var(--ink)',
              border: `1px solid ${F1_RED}66`,
              borderRadius: 6,
        ...
        live={(
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', textTransform: 'uppercase' }}>
            FORMULA 1 · 2026 · ROUND {String(gp.round).padStart(2, '0')}
            {isNext && <span style={{ color: F1_RED, fontWeight: 700 }}>· {lang === 'id' ? 'BERIKUTNYA' : 'NEXT'}</span>}
            {gp.sprint && (
              <span style={{
                padding: '1px 5px', background: F1_RED, color: '#fff',
                borderRadius: 2, fontSize: 8, fontWeight: 700, letterSpacing: 0.4,
              }}>SPRINT</span>
            )}
          </span>
        )}
        actions={(
          <HubActionRow
            ...
            shareText={shareText}
            accent={F1_RED}
            analyticsEvent="f1_race_share_strip"
          />
        )}
      />
    );
    return () => setTopbarSubrow(null);
  }, [gp, isNext, shareText, lang]);
    ...
    <div style={{ background: C.bg, minHeight: '100vh', color: C.text, fontFamily: '"JetBrains Mono", monospace' }}>
      ...
      <div className="dashboard-wrap">
        ...
        {/* v0.20.0 Phase 2 Sprint F — visible 200px hero stripped.
            Eyebrow + 36px h1 + circuit/country meta + race-time
            line + ShareButton (with igStory popover) all collapsed
            into <HubStatusStrip>. Race-info card below now leads
            the page. The 1080×1920 IG story PNG affordance is
            preserved on completed races via the per-recap card
            further down the body — not the strip. */}
        <div style={{ padding: '14px 20px 6px', fontSize: 12, color: C.dim }}>
          {gp.circuit} · {countryLabel} · {lang === 'id' ? 'Race' : 'Race'}: {formatGPDate(gp.dateISO, lang)} · {gp.wibTime} WIB
        </div>
        ...
        {/* v0.13.0 — adjacent-race peer nav (window of ±3 rounds + 6
            following rounds = 12 rows max). Lets users jump from one
            GP to the next in one click; spreads PageRank across the
            calendar. */}
        <div style={{ padding: '0 20px' }}>
          <PeerNav
            title={lang === 'id' ? 'Grand Prix lainnya' : 'Other Grands Prix'}
            currentSlug={gp.slug}
            items={CALENDAR_2026.map((race) => ({
              slug: race.slug,
              name: race.name,
              short: `R${String(race.round).padStart(2, '0')}`,
              color: F1_RED,
              href: `/formula-1-2026/race/${race.slug}`,
            }))}
            maxItems={23}
          />
        </div>
