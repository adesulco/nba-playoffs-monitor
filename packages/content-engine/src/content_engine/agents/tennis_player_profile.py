"""Tennis player profile writer (Sonnet 4.6)."""
...
def _load_system_prompt() -> str:
    return (_PROMPTS_DIR / "tennis-player-profile-system.md").read_text(encoding="utf-8")
...
def format_user_message(ctx: dict[str, Any]) -> str:
    def fld(k: str, default: str = "—") -> str:
        ...
        if v is None or v == "":
            return default
        return str(v)
    return f"""Tulis profil petenis berikut:

TOUR: {fld("tour", default="ATP").upper()}
...
async def write_tennis_player_profile(ctx: dict[str, Any]) -> dict[str, Any]:
    ...
    log.info(
        "tennis_player_profile.start",
        athlete=ctx.get("athlete_name"),
        tour=ctx.get("tour"),
    )

    result = await run_messages(
        agent="tennis-player-profile-writer",
